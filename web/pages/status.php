			...
			<div class="row">
				<div class="col-md-3">
					<div class="btn-group" role="group" aria-label="...">
						<a type="button" class="btn btn-default" href="status.php?top=<?php echo $prevPageTop.$url;?>">&lt; <?php echo L_PREV_PAGE;?></a>
						<a type="button" class="btn btn-default" href="status.php?top=<?php echo $nextPageTop.$url."&prevtop=$top";?>"><?php echo L_NEXT_PAGE;?> &gt;</a>
					</div>
				</div>
				<div class="col-md-9">
...
//							$resUrl = "{$res}<span class='label label-{$JUDGE_ROW_CSS_CLASS[$row['result']]}'>{$JUDGE_RESULT[$row['result']]}</span>{$passRate}";
							$resUrl = "{$res}<span class='label label-{$JUDGE_ROW_CSS_CLASS[$row['result']]}'>{$JUDGE_RESULT[$row['result']]}</span>";//不显示百分比
						...
						?>
                            ...
                            ?>
<!--                            class='{$JUDGE_ROW_CSS_CLASS[$row['result']]}'-->
                            <tr <?php echo "data-sid='{$row['solution_id']}'"; ?>>
