<?php
	...
	$except_user 	=@defined_or_die($_POST['except_user']);
	...
	$sql_str="DELETE FROM `privilege` WHERE `rightstr`='c$cid' AND `defunct`='N'";
	$affectedRowCnt = $pdo->exec($sql_str);
	if ($affectedRowCnt > 0) echo "Delete ".$affectedRowCnt." rows from database.<br/>";
	...
	$sql_str="DELETE FROM `privilege` WHERE `rightstr`='e$cid'";
	$affectedRowCnt = $pdo->exec($sql_str);
	if ($affectedRowCnt > 0) echo "Delete ".$affectedRowCnt." rows from database.<br/>";

	$except_user_array = explode("\n", trim($except_user)); // TODO: use array_filter() to remove the empty element
	if (count($except_user_array)>0 && strlen($except_user_array[0])>0){
		$sql_str="INSERT INTO `privilege`(`user_id`,`rightstr`) 
				VALUES ('".trim($except_user_array[0])."','e$cid')";
		for ($i=1;$i<count($except_user_array);$i++)
			$sql_str=$sql_str.",('".trim($except_user_array[$i])."','e$cid')";
		$affectedRowCnt = $pdo->exec($sql_str);
		if ($affectedRowCnt > 0) echo "Insert ".$affectedRowCnt." rows to Privilege database for except member of a contest.<br/>";
	}
