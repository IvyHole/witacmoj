	...
	*/
	...
	switch($action) {
		...
		case 'postthread':
			...
			$title = RemoveXSS(htmlspecialchars($_POST['title']));
			$content = UBB2Html(RemoveXSS((htmlspecialchars($_POST['content']))));
			...
			if($pid != null && $pid != 0) {
				if($cid != null && $cid != 0) {
					$sqlStr.=" FROM `contest_problem` WHERE `contest_id` = {$cid} AND `num` = '{$pid}'";
				} else {
			...
			fire(200, "OK", array("tid"=>$tid,"cid"=>$cid));
			break;
