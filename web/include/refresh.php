...
$logFilePath = $OJ_PATH . '/log/';//日志记录文件保存目录
$fileht = $logFilePath.'.forbidip';//被禁止的ip记录文件
$allowtime = 5;//防刷新时间
$allownum = 10;//防刷新次数
$allowRefresh = 20;//在允许刷新次数之后加入禁止ip文件中
    ...
    if ($time - $timetem < $allowtime) {
        if ($iptem != $checkip) {
            ...
            if ($uritem != $checkuri) {
                ...
                $str .= $iptem . $uritem . $timetem . (intval($numtem) + 1) . "\r\n";
            } else {
                if (!file_exists($fileforbid)) {
                ...
                $message = '警告：不要刷新的太频繁！刷新频率过快将会被拉入黑名单。';
                exit($message);
            }
        }
    }
