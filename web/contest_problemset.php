...
require_once('./include/common_const.inc.php');
	...
	if($OJ_MEMCACHE){
		...
		left join (
			SELECT problem_id pid1,count(1) accepted FROM solution WHERE result=4 AND contest_id={$cid} GROUP BY pid1
		) p1 on problem.pid=p1.pid1
        left join (
			SELECT problem_id pid2,count(1) submit FROM solution WHERE contest_id={$cid} GROUP BY pid2
		) p2 on problem.pid=p2.pid2
		ORDER BY pnum";

		$problemList=mysql_query_cache($sql);
		$problemCount=count($problemList);

		if(isset($_SESSION['user_id'])) {
			$user_id = $_SESSION['user_id'];
			$sql="SELECT `problem_id` FROM `solution` WHERE `contest_id` ={$cid} AND `result` = '4' AND `user_id` ={$user_id} GROUP BY `problem_id`";
			$acceptedList=mysql_query_cache($sql);
